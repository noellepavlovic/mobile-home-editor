...
const CallToAction: React.FC<CallToActionProps> = ({ element, index }) => {
  ...
  return (
    <div className="p-2 mb-4">
      <h3 className="font-bold text-2xl mb-4">"Call to Action" Element Editor</h3>
...
>
  ...
  <h2 className={`mb-3`} style={{ color: titleColour, fontSize: titleFontSize,}}>{title}</h2>
  <p style={{ color: descriptionColour, fontSize: descriptionFontSize }}>
    {description}
  </p>
  <button
    style={{ backgroundColor: buttonColour, color: textColour }}
    onClick={() => window.open(link, "_blank")}
    className="px-4 py-2 mt-6 rounded shadow"
  >
    {label}
  </button>
</div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-lg w-[300px]">
            ...
            {editingField === "title" || editingField === "description" ? (
              <>
                ...
                {hexInputError && (
                  <p className="text-red-500 text-sm">{hexInputError}</p>
                )}
