...
const Carousel: React.FC<CarouselProps> = ({
  ...
  const handleNewImageUrlChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleEditImageUrlChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!editImage) return;
    const value = e.target.value;
  ...
  if (isPreviewing) {
    return (
      <PreviewContainer element={element} onClose={() => setIsPreviewing(false)} />
    );
  }
            ...
            >
              ...
              {!isPreview && (
                <div className="flex flex-col justify-between h-auto min-h-[125px] max-w-[300px] min-w-[100] w-auto absolute bottom-4 left-0  m-5 bg-white p-2 rounded shadow">
                  ...
                  <p className="text-xs text-gray-500 mb-4">{image.description}</p>
                  <button
                    onClick={() => {
        ...
        {showEditModal && editImage && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
              ...
              />
              {editImageUrlError && (
                <p className="text-rose-500 text-sm mb-2">{editImageUrlError}</p>
              )}
              {!editImageUrlError && editImage.url && (
