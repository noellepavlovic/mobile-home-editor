...
const ItemContainer: React.FC<{ element: any; index: number }> = ({ element, index }) => {
  ...
  const [, drop] = useDrop(() => ({
    ...
    hover: (draggedItem: { id: string; type: string; index?: number }) => {
      if (draggedItem.index === undefined || draggedItem.index === index) return;
  ...
  const renderElement = () => {
    switch (element.type) {
      ...
      case "carousel":
        return <Carousel element={element} onUpdate={(updatedConfig) => {
          const updatedElements = [...elements];
